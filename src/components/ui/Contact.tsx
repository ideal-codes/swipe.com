            ...
            {!submitted ? (
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="space-y-1">
                  <h3 className="text-xl font-bold font-display text-white">Project Inquiry Form</h3>
                  <p className="text-xs font-mono text-[#A8A8A8]">Direct response guaranteed within 4 hours</p>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs font-mono text-[#A8A8A8] mb-1.5 block">Full Name *</label>
                    <input
                      type="text"
                      required
                      value={formData.name}
                      ...
                      className="w-full px-4 py-3 rounded-xl bg-[#171717] border border-white/10 text-white text-xs font-mono focus:border-[#E61E4D] focus:outline-none"
                    />
                  </div>

                  <div>
                    <label className="text-xs font-mono text-[#A8A8A8] mb-1.5 block">Work Email *</label>
                    <input
                      type="email"
                      required
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs font-mono text-[#A8A8A8] mb-1.5 block">Company Name</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="text-xs font-mono text-[#A8A8A8] mb-1.5 block">Estimated Budget</label>
                    <select
                ...
                <div className="space-y-2 pt-2 border-t border-white/5">
                  <label className="text-xs font-mono text-[#A8A8A8] uppercase tracking-wider block flex items-center gap-1.5">
                    ...
                    {timeSlots.map((slot) => (
                      <button
                        ...
                        className={`py-2 rounded-lg text-[11px] font-mono transition-all ${
                          selectedSlot === slot
                            ? 'bg-[#E61E4D] text-white font-bold shadow-md shadow-[#E61E4D]/30'
                ...
                <div>
                  <label className="text-xs font-mono text-[#A8A8A8] mb-1.5 block">Project Summary / Requirements *</label>
                  <textarea
                    rows={4}
                    required
                ...
                <button
                  type="submit"
                  className="w-full py-4 rounded-xl bg-[#E61E4D] hover:bg-[#FF2D55] text-white text-xs font-mono font-bold uppercase tracking-wider transition-all shadow-xl shadow-[#E61E4D]/30 flex items-center justify-center gap-2"
                >
              ...
              <div className="py-12 text-center space-y-6">
                ...
                <div className="space-y-2">
                  <h3 className="text-2xl font-bold font-display text-white">Consultation Confirmed!</h3>
                  <p className="text-xs font-mono text-[#A8A8A8] max-w-md mx-auto">
                    We have reserved your 30-min slot for <span className="text-white font-bold">{selectedSlot}</span>. A meeting invitation has been sent to <span className="text-[#E61E4D]">{formData.email}</span>.
                  </p>
                </div>
                <button
                  onClick={() => setSubmitted(false)}
                  className="px-6 py-2.5 rounded-xl bg-white/10 hover:bg-white/20 text-xs font-mono text-white font-bold"
                >
